...
> 本文转自[wuyangchun](https://segmentfault.com/a/1190000009249039)

### 虚拟网络设备

Linux用户想要使用网络功能，不能直接通过操作硬件完成，而需要直接或间接的操作一个通过Linux网络设备管理层为我们抽象出来的设备，既通用的Linux网络设备来完成。一个常见的情况是，系统里装有一个硬件网卡，Linux会在系统里为其生成一个网络设备实例，如eth0，用户通过对eth0发出命令进而使用硬件设备。同时虚拟的网络设备（tun/tap）也会对应一个网络设备实例如tun0。

网络设备管理层处于网络设备驱动和协议栈之间，负责衔接它们之间的数据交互。驱动不需要了解协议栈的细节，协议栈也不需要了解设备驱动的细节。

对于一个网络设备来说，就像一个管道（pipe）一样，都有两端，从其中任意一端收到的数据将从另一端发送出去。
...
当一个Tun/Tap设备被创建时，在 Linux设备文件目录下将会生成一个对应char设备，用户程序可以像打开普通文件一样打开这个文件进行读写。当执行write()操作时，数据进入Tun/Tap设备，此时对于Linux网络层来说，相当于Tun/Tap设备收到了一包数据，请求内核接受它，如同普通的物理网卡从外界收到一包数据一样，不同的是其实数据来自Linux上的一个用户程序。Linux 收到此数据后将根据网络配置进行后续处理，从而完成了用户程序向Linux内核网络层注入数据的功能。当用户程序执行 read()请求时，相当于向内核查询Tun/Tap设备上是否有需要被发送出去的数据，有的话取出到用户程序里，完成Tun/Tap设备的发送数据功能。针对Tun/Tap设备的一个形象的比喻是：使用 Tun/Tap设备的应用程序相当于另外一台计算机，Tun/Tap设备是本机的一个网卡，他们之间相互连接。应用程序通过read()/write()操作，和本机网络核心进行通讯。

Tun/Tap都是虚拟网卡，没有直接映射到物理网卡，是一种纯软件的实现。Tun是三层虚拟设备，能够处理三层即IP包，Tap是二层设备，能处理链路层网络包如以太网包。使用虚拟网络设备，常用于实现隧道以及平时网络调试。
...
* [3] https://www.ibm.com/developerworks/cn/linux/1310_xiawc_networkdevice/index.html